...
TAEHVDecoder handles the temporal taehv format (madebyollin/taehv), which core builds but
can't size for H3 — this is what makes "taeh3" (a taehv checkpoint trained for MiniMax
H3's 24-channel, patch-size-2 packed video latent) work: `is_h3` detects that shape and
switches to `_decode_h3_full`, H3's own 17-pixel-frames-per-5-latent-tokens chunking, so a
whole-clip decode comes out at the exact frame count H3 will actually output.
"""
